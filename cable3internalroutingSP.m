...
syms L1 L2 b1 b2 b3 b4 d1 d2 d3 d4 h1 h2 h3 h4 u1 u2 u3 u4 s1 s2 s3 s4 s5 s11
...
s1=10e-3; s2=10e-3; s3=10e-3; s4=10e-3; s5=10e-3; s11=10e-3;
...
%point
%3muscle external routing for reference
% q1= sqrt((h1+r_a*Ca-s1*Sa)^2+(r_a*Sa+s1*Ca)^2);
% q2= sqrt((-h2+r_a*Ca+s2*Sa)^2+(r_a*Sa-s2*Ca)^2);
% q3= sqrt((u1+r_a*Ca-s1*Sa)^2+(r_a*Sa+(s1+s5)*Ca)^2);
% q4= sqrt((d1*Ca+ s1*Sa+ r_b*Cab-s4*Sab)^2+(d1*Sa-s1*Ca+ r_b*Sab + s4*Cab)^2);
% q5= sqrt((d1*Ca- s2*Sa+ r_b*Cab+s3*Sab)^2+(d1*Sa+s2*Ca+ r_b*Sab - s3*Cab)^2);
% qc1=q1+q4;
% qc2=q2+q5;
% qc3=q3;
% 3 muscle internal routing generic case with m1 m2.. offset at the base is
% equal to zero units.
q11= sqrt((-h1+ra1*Ca-s11*Sa)^2+(ra1*Sa-s11*Ca-m1)^2);
q1int= sqrt((-h1+ra2*Ca+s11*Sa+s1*Sa)^2+(ra2*Sa-s11*Ca-s1*Ca-m1)^2);
q12= sqrt((d1*Ca+ s1*Sa+ r_b*Cab-s4*Sab)^2+(d1*Sa-s1*Ca+ r_b*Sab + s4*Cab)^2);
q2= sqrt((-h2+r_a*Ca+s2*Sa)^2+(-m2+r_a*Sa-s2*Ca)^2);
q3= sqrt((u1+r_a*Ca-s1*Sa)^2+(-p1+r_a*Sa+(s1+s5)*Ca)^2);
%q4= sqrt((d1*Ca+ s1*Sa+ r_b*Cab-s4*Sab)^2+(d1*Sa-s1*Ca+ r_b*Sab + s4*Cab)^2);
q5= sqrt((d1*Ca- s2*Sa+ r_b*Cab+s3*Sab)^2+(d1*Sa+s2*Ca+ r_b*Sab - s3*Cab)^2);
qc1=q11+q12+q1int;
qc2=q2+q5;
...
save('CN_cable3internalroutingSP.mat')